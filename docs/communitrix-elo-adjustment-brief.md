...
**IMPORTANT FRAMING CORRECTION:** the previous version of this section called the gap below a "critical bug." It isn't — **Communitrix is still in active development**, and per the product owner: Quick Match was deliberately built first, as a sandbox to nail down the scoring/UX experience (`wizard-form.tsx`'s local-state, instant-feedback approach), with the explicit plan to **replicate that experience into the real community session flow afterward**, adjusted and extended with everything in this brief (Elo, Skill Rating, Community Points, etc.). So `wizard-form.tsx` doing everything in local React state isn't a mistake — it's an intentional first step, not yet followed by step two.

**What actually needs to be built (this is real, unstarted scope, not a bug fix):** a community-session version of the session flow that:
1. Reuses the wizard's UX (bye-point display, live standings, podium, score entry) — this part is already proven and works well in the sandbox.
2. But is wired to the **already-existing, already-correct, round-by-round database flow**: `generateNextRoundAction` (server-side, pulls live `session_players` standings from Supabase, calls the same `generateAmericanoRound`/`generateMexicanoRound` functions the wizard already uses) → user enters scores as each match finishes → a **`submitMatchScoreAction` wrapper around `submit_match_score` needs to be created — it does not exist yet** (confirmed: not present in either `session.actions.ts` or `round.actions.ts`) → repeat per round → `finalizeSessionAction` (already exists) at the end.
3. This is the point where every patch in this brief (Elo, Skill Rating review triggers, Community Points) actually starts having an effect — none of it fires until real match scores reach `submit_match_score`.
...
**Still open, this is a real architecture/build task, not something resolved in this session:**
1. `submitMatchScoreAction` needs to be created (wrapping `submit_match_score`, following the exact pattern of `persistRoundAction`/`finalizeSessionAction` in the same files).
2. Whether the community version keeps the wizard's bye-points feature at all, and if so, whether it's computed the same way (client-side estimate) or needs to be a real, persisted mechanic — not decided, out of scope for the rating patches below, but worth deciding before this build starts.
3. The actual UI/UX rebuild connecting `generateNextRoundAction` → per-match score entry → `submitMatchScoreAction` → repeat is a genuine feature-build project, not a brief section — recommend treating it as its own implementation task for Antigravity, with this brief's Patches 1-9 as the rating logic that hooks into `submit_match_score`/`finalize_session` once that flow exists.
