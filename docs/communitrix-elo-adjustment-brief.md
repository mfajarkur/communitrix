...
**STATUS UPDATE (Patch 5 + Patch 9 shipped as formula_version 2):** Carry Rule (Patch 5, section 6) and Effective Team Rating (Patch 9, section 8) are now implemented together, gated behind `matches.formula_version >= 2`, in `supabase/migrations/0028_carry_rule_and_effective_team_rating.sql` and `src/lib/elo/calculate.ts`. Every match created from here on gets the currently-active version (looked up from `rating_formula_versions` at creation time in `persist_round`); matches created before this shipped keep `formula_version = 1` forever, including on replay via void/amend, so historical results never silently shift.

**Important correction to the rest of this document below:** while implementing this, `migration 0018_elo_adjustments_and_cp.sql` (which this brief's Patches 1-9 are written against, including `calculate_match_delta`, `formula_version`, `rating_formula_versions`, Skill Rating, and Community Points) was found to **never have actually been applied to the live database** — none of its tables, columns, or functions existed. Only `submit_match_score`'s later rewrite in `0025_fix_claim_rls_and_round_gender_conflict.sql` (raw team-average, no gap penalty, no Carry Rule — effectively a from-scratch v1 implementation, coincidentally consistent with this brief's own "v1" description) was ever live. `0028` therefore creates `formula_version`/`rating_formula_versions` fresh rather than building on 0018, and only implements Patch 5 + Patch 9. **Update:** Patch 7 (Community Points) has since also been implemented fresh, in `0029_community_points.sql` — see section 9. Patches 1 (SQL dedup via `calculate_match_delta`), 3 (remove `amend_match_score`), 4 (new-vs-new dampening), 6 (Skill Rating), and 8 (Round 1 seeding blend) remain just proposals, not implemented, regardless of what any given section's own status text below might otherwise imply.

**Also found and fixed while implementing this:** `replay_ratings` (used by both `void_match` and `amend_match_score`) has been broken since it first shipped in `0011_rpc_amend_void.sql` — its query filtered on `matches.sport`, a column that has never existed (`sport` lives on `sessions`). Every call failed with `column "sport" does not exist`, meaning voiding or amending a match's score has never actually worked in production. Fixed in `0028` by joining `sessions` for the sport filter instead.
...
**STATUS: implemented**, in `supabase/migrations/0029_community_points.sql` — `community_points`, `community_point_seasons`, `communities.cp_reset_policy`, `calculate_cp_points`, `start_new_cp_season`, and the CP-awarding block appended to `finalize_session`, exactly per this section's design (including the podium/cliff/decay formula, the `never`/`seasonal` reset policy, and the standings_metric → seed_elo → profile_id tiebreak). `community-tabs.tsx` was already reading/displaying a `cpMap` built from this table and already had wiki text describing this exact formula — it was just never backed by real data until now. `start_new_cp_season` has a working RPC as of this migration, but still has no UI control anywhere to call it (`startNewCpSeasonAction` in `session.actions.ts` is unused by any component) — building that toggle is a separate, not-yet-requested follow-up.
