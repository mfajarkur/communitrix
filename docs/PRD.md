...
### 17.2 Navigation/IA redesign: global bottom nav, status ribbon, community count limits

- **What**: the old navigation mixed two inconsistent patterns — a flat, scrolling `/communities` page (profile + community list + Quick Match all in one) and a separate bottom-tab-bar pattern once inside a specific community (`/c/[slug]`). Replaced with one consistent shell: a persistent global bottom nav with three destinations — **Profile · Community · Activities** — plus a persistent branding/status ribbon at the very top of every authenticated page.
- **Global bottom nav** (`src/components/global-nav/bottom-nav.tsx`, mounted in `src/app/(app)/layout.tsx`): Profile → `/profile`, Community → `/communities` (list only now), Activities → `/activities` (redirects to `/activities/sessions`). The Community icon stays highlighted for both `/communities*` and `/c/*` so it keeps reading as "you're still in the Community space" while inside a specific community.
- **Status ribbon** (`src/components/status-ribbon/`): replaces the old route-nav-only `navigation-loader.tsx`. `StatusRibbonProvider` exposes `useStatusRibbon()` (`showStatus(message, id?) → id`, `clearStatus(id)`) so any async action anywhere in the app can push a busy message onto the ribbon, not just route navigation — route detection is now just the first caller of its own API. Idle state shows the "communitrix.id" wordmark; busy state grows an orange fill bar literally empty→full and swaps in a light-weight status message. Wired into score submission and session finalization (`live-board-wrapper.tsx`) and profile save (`profile/settings-sheet.tsx`) as the first adopters beyond route navigation; other local busy indicators (claim requests, member actions, round generation) were deliberately left alone in this pass.
- **Profile tab** (`src/app/(app)/profile/`): rebuilt from the old combined `/communities` page's `ProfileCard` — circular avatar, centered layout, gender as a static badge (editable only from the new Settings sheet, not a live toggle), stats split into Tennis/Padel sections (`getMyPerSportStats`), and a per-sport Elo trend chart (`getMyEloTrend`, `elo-trend-chart.tsx`) reconstructed from existing `match_players`/`matches`/`sessions` history — no new schema. Settings and Help now live in bottom sheets (`settings-sheet.tsx`, `help-sheet.tsx`), portal-rendered to `document.body` rather than positioned against the scrollable content region, to avoid a sheet-positioning class of bug.
- **Community tab** (`src/app/(app)/communities/`): list-only now (Profile and Quick Match moved out). New business rule: **accounts are capped at 3 created communities and 5 joined (active) communities**, enforced server-side inside the `create_community`/`join_community` RPCs (`supabase/migrations/0035_community_membership_limits.sql`) so the cap is a real trust boundary, not just client-side UX. The UI shows a quiet "X/3 created · Y/5 joined" chip row (`usage-chips.tsx`) and pre-empts an upgrade prompt (`upgrade-modal.tsx`) before navigating to create/join — no "Free Plan" label anywhere, since the free/paid tier system itself isn't being announced yet. Communitrix Plus (pricing, what it unlocks) is explicitly out of scope — only the numeric caps and the upgrade CTA exist so far.
- **Activities tab** (`src/app/(app)/activities/`): new top-level destination with two sub-tabs. **My Session** (`activities/sessions/`) is a brand-new cross-community session list (`getMySessions()` in `session.actions.ts`) showing every session — live or ended — the caller has played in across all their communities; CANCELLED and DRAFT sessions are excluded. **Quick Match** (`activities/quick-match/`) is the existing Personal Quick Match feature relocated from `/communities/quick-match*` (engine/scoring untouched, pure route move); old URLs redirect via `next.config.ts`.
- **In-community view** (`src/app/(app)/c/[communitySlug]/community-tabs.tsx`): the old bottom-fixed Home/Sessions/Members/Rank nav moved to a top strip under a new compact header (thumbnail + name + role badge + "← Communities" breadcrumb), so only the global bottom nav ever occupies the bottom of the screen. The old Home-only hero banner's admin tools (banner/info editors) and quick stats now live inside the Home tab's own content as an "About" card instead of a page-wide banner.