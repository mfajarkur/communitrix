...
- **Match Type (`sessions.match_type`)**: Padel is always Doubles (2 vs 2) — enforced by a DB check constraint. Tennis can be either Doubles or Singles (1 v 1), chosen by the host at session creation; the session wizard only offers the choice for Tennis. Respected by both Online and Offline community sessions. Quick Match (the standalone sandbox) always plays Doubles — the choice isn't surfaced there.
...
- `supabase/migrations/0032_session_match_type.sql`: wires `sessions.match_type` (existed since the original schema, but `start_session` never had a parameter for it) into `start_session`. `persist_round`/`submit_match_score`/`replay_ratings` (0028) and the matchmaking engines (`americano.ts`/`mexicano.ts`) already handled Singles correctly at the data-shape level. Two client-side entry points still needed fixing on top of the migration: `round.actions.ts`'s Online round generator (hardcoded Doubles) and `wizard-form.tsx`'s local engine shared by Quick Match/Offline (also hardcoded, plus one unfiltered team-name join that could show a phantom second player) — `uploadOfflineSessionAction`'s own `start_session` call needed `p_match_type` threaded through too, since Offline sessions only reach the database at upload time.
