...
export default async function AppLayout({
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen bg-zinc-50 dark:bg-zinc-950 w-full">
      {/* Mobile Header (rendered consistently on all devices) */}
      <header className="flex items-center justify-between border-b border-zinc-200 bg-white px-5 py-3.5 dark:border-zinc-800 dark:bg-zinc-900 shrink-0 w-full">
        <Link href="/communities" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
          <Trophy className="h-5 w-5 text-indigo-600" />
          <span className="font-semibold text-zinc-950 dark:text-white text-sm">Communitrix</span>
        </Link>
